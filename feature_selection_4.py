...
Fitness is evaluated using Logistic Regression on the masked feature subset.
...
def _fitness(mask, X_train, y_train, X_val, y_val) -> float:
    """
    ...
    clf = LogisticRegression(max_iter=200, solver="lbfgs", n_jobs=-1, C=1.0)
    clf.fit(X_tr, y_train)
    acc = accuracy_score(y_val, clf.predict(X_v))
